...
export const SalesCitiesChart: FC = () => {
  ...
  const salesAnalyticsResults =
    salesAnalytics?.state_sales[0]?.city_sales.slice(0, 10)
  ...
  const data: ChartDataType = {
    labels: salesAnalyticsResults?.map(({ city }) => city.name),
    datasets: [
      {
        label: "Orders Count",
        data: salesAnalyticsResults?.map((city) => city.orders_count) || [],
  ...
  if (!isRequestEnabled || !salesAnalyticsResults?.length) {
    ...
    if (!isRequestEnabled) {
      message = "Select Shop, Country, State and period to see chart"
    }
